...
class TestMisc extends Test {
	...
	function testHiddenType() {
		var hydr = 20;
		eq( std.hydr.crypto.Md5.encode(""), "d41d8cd98f00b204e9800998ecf8427e");
		eq( hydr, 20);
		var Std = 50;
	...
	function testMD5() {
		eq( hydr.crypto.Md5.encode(""), "d41d8cd98f00b204e9800998ecf8427e" );
		eq( hydr.crypto.Md5.encode("hello"), "5d41402abc4b2a76b9719d911017c592" );
		// depending of ISO/UTF8 native
		allow( hydr.crypto.Md5.encode("héllo"), ["1a722f7e6c801d9e470a10cb91ba406d", "be50e8478cf24ff3595bc7307fb91b50"] );

		eq( hydr.io.Bytes.ofString("héllo").toHex(), "68c3a96c6c6f");
		eq( hydr.crypto.Md5.make(hydr.io.Bytes.ofString("héllo")).toHex(), "be50e8478cf24ff3595bc7307fb91b50" );
	}

	function testSHA1() {
		eq( hydr.crypto.Sha1.encode(""), "da39a3ee5e6b4b0d3255bfef95601890afd80709" );
		eq( hydr.crypto.Sha1.encode("hello"), "aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d" );
		// depending of ISO/UTF8 native
		allow( hydr.crypto.Sha1.encode("héllo"), ["028db752c14604d624e8b1c121d600c427b8a3ba","35b5ea45c5e41f78b46a937cc74d41dfea920890"] );

		eq( hydr.crypto.Sha1.make(hydr.io.Bytes.ofString("héllo")).toHex(), "35b5ea45c5e41f78b46a937cc74d41dfea920890" );
	}

	function testBaseCode() {
		var b = new hydr.crypto.BaseCode(hydr.io.Bytes.ofString("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-"));
...
//
	...
	function testJSon() {
		var str = hydr.Json.stringify( { x : -4500, y : 1.456, a : ["hello", "wor'\"\n\t\rd"] } );
		...
		function id(v:Dynamic,?pos:hydr.PosInfos) eq(hydr.Json.parse(hydr.Json.stringify(v)),v);
		function deepId(v:Dynamic) {
			var str = hydr.Json.stringify(v);
			eq(hydr.Json.stringify(hydr.Json.parse(str)), str);
		}
		...
		eq( hydr.Json.parse('"\\u00E9"'), "é" );

		eq(hydr.Json.stringify(Math.POSITIVE_INFINITY), "null");
		eq(hydr.Json.stringify(Math.NEGATIVE_INFINITY), "null");
		eq(hydr.Json.stringify(Math.NaN), "null");
	}
