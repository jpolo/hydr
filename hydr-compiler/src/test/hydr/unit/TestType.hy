...
class TestType extends Test {
	...
	static public macro function typedAs(actual:hydr.macro.Expr, expected:hydr.macro.Expr) {
		var tExpected = hydr.macro.Context.typeof(expected);
		var tActual = hydr.macro.Context.typeof(actual);
		return hydr.macro.Context.parse("{Test.count++; eq('" +Std.string(tActual) + "', '" +Std.string(tExpected) + "');}", hydr.macro.Context.currentPos());
	}

	static public macro function typeError(e:hydr.macro.Expr) {
		var result = try {
			hydr.macro.Context.typeof(e);
			"false";
		} catch (e:Dynamic) "true";
		return { pos: hydr.macro.Context.currentPos(), expr: hydr.macro.Expr.ExprDef.EConst(hydr.macro.Expr.Constant.CIdent(result)) };
	}

	static public macro function complete(e:String) : hydr.macro.Expr.ExprOf<String> {
		var str = new String(untyped hydr.macro.Context.load("display", 1)(e.__s));
		return { expr : EConst(CString(str)), pos : hydr.macro.Context.currentPos() };
	}
	...
	function testCallback()
	{
		...
		var foo = function ( x : Int, ?p : hydr.PosInfos ) { return "foo" + x; }
	...
	public function testGenericFunction() {
		...
		gf1(new hydr.Template("foo"));
		#end

		gf1(new hydr.ds.GenericStack<Int>());
		hsf(TestType, "gf1_Int");
		hsf(TestType, "gf1_String");
		hsf(TestType, "gf1_Bool");

		#if !flash8
		hsf(TestType, "gf1_hydr_Template");
		#end

		hsf(TestType, #if (flash9 || cpp) "gf1_hydr_ds_GenericStack_Int" #else "gf1_hydr_ds_GenericStack" #end);
		...
		#if !flash8
		var t = new hydr.Template("foo");
		var ta = gf3(t, [])[0];
		f(t == ta);
		hsf(TestType, "gf3_hydr_Template_Array");
		#end
	...
	function testCompletion() {
		...
		eq(complete("var x : hydr.|"), "path(hydr)");
		eq(complete("var x : hydr.macro.Expr.|"), "path(hydr.macro:Expr)");

		// could be improved by listing sub types
		eq(complete("hydr.macro.Expr.|"), "error(hydr.macro.Expr is not a value)");
	...
	function testVoidFunc() {
		...
		#if !flash8
		exc(function() { throw null; return new hydr.Template("foo"); } );
		#end
	...
	function testOpArrow() {
		...
		var map = ["1" => 2, "3" => 4];
		typedAs(map, m);
		t(Std.is(map, hydr.ds.StringMap));
		eq(map.get("1"), 2);
		eq(map.get("3"), 4);
	...
	function testAbstractGeneric() {
		...
		//t(Std.is(map, hydr.ds.ObjectMap));

		//var map = new unit.MyAbstract.MyMap();
		//map.set(new hydr.Template("foo"), 99);
