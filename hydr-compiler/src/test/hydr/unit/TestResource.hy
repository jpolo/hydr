...
class TestResource extends Test {
	...
	function testResources() {
		var names = hydr.Resource.listNames();
		eq( names.length, 2 );
		if( names[0] == "res1.txt" )
			eq( names[1], "res2.bin" );
		else {
		...
		eq( hydr.Resource.getString("res1.txt"), STR );
		#if (neko || php)
		// allow binary strings
		eq( hydr.Resource.getBytes("res2.bin").sub(0,9).toString(), "MZ\x90\x00\x03\x00\x00\x00\x04" );
		#else
		// cut until first \0
		eq( hydr.Resource.getString("res2.bin").substr(0,2), "MZ" );
		#end
		eq( hydr.Resource.getBytes("res1.txt").compare(hydr.io.Bytes.ofString(STR)), 0 );
		var b = hydr.Resource.getBytes("res2.bin");
