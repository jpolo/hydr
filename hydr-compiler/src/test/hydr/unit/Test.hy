...
class Test #if swf_mark implements mt.Protect #end {
	...
	function async<Args,T>( f : Args -> (T -> Void) -> Void, args : Args, v : T, ?pos : hydr.PosInfos ) {
	...
	function asyncExc<Args>( seterror : (Dynamic -> Void) -> Void, f : Args -> (Dynamic -> Void) -> Void, args : Args, ?pos : hydr.PosInfos ) {
		if( asyncWaits.length >= AMAX ) {
	...
	function log( msg, ?pos : hydr.PosInfos ) {
		hydr.Log.trace(msg,pos);
	}
	...
	static var asyncWaits = new Array<hydr.PosInfos>();
	static var asyncCache = new Array<Void -> Void>();
	static var AMAX = 3;
	static var timer : hydr.Timer;

	dynamic static function report( msg : String, ?pos : hydr.PosInfos ) {
		if( reportInfos != null ) {
			msg += " ("+reportInfos+")";
			reportInfos = null;
		}
