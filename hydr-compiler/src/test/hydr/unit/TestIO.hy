...
class TestIO extends Test {
	...
	function check(endian:Bool) {
		...
		var b = hydr.io.Bytes.ofString("ABCééé\r\n\t");
		eq( b.length, 12 );
		b.set(1,0);

		var o = new hydr.io.BytesOutput();
		...
		var i = new hydr.io.BytesInput(o.getBytes());
		i.bigEndian = endian;
		...
		eq( i.readString(hydr.io.Bytes.ofString(str).length), str );
	...
	function testBytesBounds() {
		var b = hydr.io.Bytes.ofString("ABCDEFGHIJ");
		var tmp = hydr.io.Bytes.alloc(7);
		var i = new hydr.io.BytesInput(b);
