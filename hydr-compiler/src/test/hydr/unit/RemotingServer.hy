...
class RemotingServer extends neko.net.ThreadRemotingServer {
	...
	override function initClientApi( cnx : hydr.remoting.SocketConnection, ctx : hydr.remoting.Context ) {
	...
	override function onXml( cnx : hydr.remoting.SocketConnection, data : String ) {
	...
	static function main() {
		if( neko.Web.isModNeko ) {
			...
			if( !hydr.remoting.HttpConnection.handleRequest(ctx) )
