...
class TestBytes extends Test {
	...
	function test() {
		...
		eq(b.get(7),0);
		// readString
		var bs = hydr.io.Bytes.ofString("One é accent");
		...
		**/
		...
		for( s1 in strings )
			for( s2 in strings ) {
				var c = hydr.io.Bytes.ofString(s1).compare(hydr.io.Bytes.ofString(s2));
		...
		infos(null);
		// sub
		var bs = hydr.io.Bytes.ofString("ABCDEFGH");
		eq( bs.sub(1,3).compare(hydr.io.Bytes.ofString("BCD")), 0 );
	...
	function testBuffer() {
		var out = new hydr.io.BytesBuffer();
		out.add( hydr.io.Bytes.ofString("ABCDEF") );
		for( i in 1...6 )
			out.addByte(i);
		out.addBytes( hydr.io.Bytes.ofString("ABCDEF"),1,3 );
		var b = out.getBytes();
	...
	function testInput() {
		var bs = hydr.io.Bytes.ofString("One é accent");
		var input = new hydr.io.BytesInput(bs);
		//readAll
