...
import hydr.test.Base;
import hydr.test.Base.Base_InnerClass;
...
class BaseJava implements NormalInterface
{
	...
	@:overload public function someField(b:hydr.io.Bytes):Int
	{
		return 0;
	}
...
{
	@:overload public function new(b:hydr.io.Bytes)
	{
		super(b.toString());
	}
	...
	@:overload override public function someField(b:hydr.io.Bytes)
	{
		return 2;
	}
}
...
class ChildJava2<T> extends ChildJava
{
	...
	@:overload private function new(b:hydr.io.Bytes)
	{
		super(b);
	}
...
class TestJava extends Test
{
  function textException()
  {
	...
	{
		t(hydr.test.GenericHelper.staticTypedGeneric(Base_InnerClass_InnerInnerClass) != null);

		var helper = new hydr.test.GenericHelper();
		//TODO use typedAs
		eq(helper.getUntypedGeneric(), null);
		eq(helper.typedGeneric, null);

		var val = new Base_InnerClass();
		var g1 = new hydr.test.Generic1(val);
	...
	function testOverloadOverride()
	{
		...
		c.normalOverload(10);
		t(c.intCalled);
		c.normalOverload(hydr.Int64.ofInt(0));
		t(c.int64Called);
		c.normalOverload("");
		t(c.stringCalled);
		c.normalOverload({});
		...
		var b:hydr.test.MyClass = c;
		b.normalOverload(true);
		t(c.boolCalled);
		b.normalOverload(10);
		t(c.intCalled);
		b.normalOverload(hydr.Int64.ofInt(0));
		t(c.int64Called);
		b.normalOverload("");
		t(c.stringCalled);
		b.normalOverload({});
...
private class TestMyClass extends hydr.test.MyClass
{
	...
	@:overload override public function normalOverload(i64:hydr.Int64):Void
	{
