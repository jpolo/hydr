...
function extract(e) {
	switch(e.expr) {
		case hydr.macro.Expr.ExprDef.EConst(hydr.macro.Expr.Constant.CString(s)):
			strings.push(s);
		case hydr.macro.Expr.ExprDef.EConst(hydr.macro.Expr.Constant.CIdent(s)) if (s.charCodeAt(0) >= 'A'.code && s.charCodeAt(0) <= 'Z'.code):
			upperIdents.push(s);
		case _:
			hydr.macro.ExprTools.iter(e, extract);
	}
}
extract(subject);
...
var iter = hydr.macro.ExprTools.iter;
...
function unwrap(e) return switch(e.expr) {
	case hydr.macro.Expr.ExprDef.EParenthesis(e): e;
	case _: e;
}
var map = hydr.macro.ExprTools.map;
